...
export function AlertWrongMediaVideoOnMediaId({ mediaTitle, mediaId, mediaFormat }: { mediaTitle: string, mediaId: number, mediaFormat: string }) {
    ...
    return (
        <div id={styles.alert_container} className="bg-gray-100 dark:bg-gray-900 text-black dark:text-white">
            ...
            <div>
                ...
                <h2>
                    The current video {`doesn't`} match any video available for <span>{mediaTitle}</span> ({mediaFormat == "TV" ? "ANIME" : mediaFormat})
                </h2>
                ...
                <div>
                    ...
                    <label>
                        {`Don't`} Show Again
                        <input type="checkbox" onChange={() => changePlayWrongMediaCookieValue()} />
                    </label>
