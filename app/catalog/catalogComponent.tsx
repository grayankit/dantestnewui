...
import CasualCard from '@/app/components/shared/CasualCard';
import { Input, RadioGroup, Radio, Button } from '@nextui-org/react';
import useDebounce from '@/app/hooks/useDebounce';
import React from 'react';
import { ExtendedAnimePage } from '@/app/lib/clientSide';
...
export default function CatalogComponent() {
  ...
  const [content, setContent] = React.useState<string>('ANIME');
  ...
  React.useEffect(() => {
    const fetchSearchResult = async () => {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_WEBSITE_ORIGIN_URL}/api/advancedSearch?${queryParams}`
      );
    ...
    fetchSearchResult();
  }, [selectedGenres, season, debouncedYear, debouncedQuery, format ,content]); // Refetch results when filters change
